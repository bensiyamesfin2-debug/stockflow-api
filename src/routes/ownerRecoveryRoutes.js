...
router.post("/", async (req, res) => {
  ...
  await prisma.$transaction([
    ...
    prisma.auditLog.create({
      data: { userId: owner.id, action: "RECOVER_PLATFORM_OWNER_ACCOUNT", entityType: "USER", entityId: owner.id },
    }),
  ]);
  return res.json({ success: true, message: "Owner account recovery completed" });
});
